...
export default function HomeScreen() {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      const cachedCats = await CacheService.getCategories();
      const localCategories = cachedCats.filter(c => c.id.startsWith('local-'));
      const uniqueFreshCategories = freshCategories.filter(
        (fc) => !localCategories.some((lc) => lc.name.toLowerCase() === fc.name.toLowerCase())
      );
      const combinedCategories = [...uniqueFreshCategories, ...localCategories];

      // Preserve local expenses
      const localExpenses = cachedExpenses.filter(e => e.id.startsWith('local-'));
      const combinedExpenses = [...freshExpenses, ...localExpenses];
  ...
  return (
    <SafeAreaView style={[styles.container, { backgroundColor: theme.colors.background }]} edges={['top']}>
      ...
      <View style={styles.sectionHeader}>
        ...
        <Button variant="ghost" size="sm" onPress={() => router.push('/(tabs)/expenses')}>
          See all
        </Button>
      </View>
      ...
      {expenses.slice(0, 5).map((expense) => (
        <View key={expense.id} style={[styles.expenseRow, { borderBottomColor: theme.colors.border }]}>
